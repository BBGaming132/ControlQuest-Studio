...
export function isAvatarItemUnlocked(profile,type,id){ const item=AVATAR_ITEMS[type]?.find(x=>x.id===id); if(!item) return false; if(profile.avatar?.owned?.includes(id)) return true; const lvl=levelFromXp(profile.stats?.xp||0).level; const streak=profile.stats?.streak||0; const qae=profile.progress?.qaeLogs?.length||0; const mistakes=profile.stats?.totalMistakes||0; const unlock=item.unlock||'Start'; if(unlock==='Start') return true; if(unlock.includes('Level 2')) return lvl>=2; if(unlock.includes('Level 3')) return lvl>=3; if(unlock.includes('Level 4')) return lvl>=4; if(unlock.includes('Level 5')) return lvl>=5; if(unlock.includes('7-day')) return streak>=7; if(unlock.includes('3-day')) return streak>=3; if(unlock.includes('5 QAE')) return qae>=5; if(unlock.includes('10 mistakes')) return mistakes>=10; if(unlock.includes('Perfect')) return (profile.stats.perfectWeeks||0)>0; return false; }
...
export function renderAvatar(profile,size='normal'){ const a=profile.avatar||{}, color=a.baseColor||'#7c4dff', cape=a.cape||'none', glasses=a.glasses||'round', accessory=a.accessory||'clipboard', mood=a.mood||'focused'; const smile=mood==='happy'?'M66 111c13 12 35 12 48 0':mood==='locked-in'?'M70 112h40':'M67 112c12 10 34 10 46 0'; const capePath=cape==='none'?'':`<path d="M42 70 C20 86 18 112 35 128 C52 112 73 111 90 128 C107 111 128 112 145 128 C162 112 160 86 138 70 Z" fill="${cape==='gold-cape'?'#ffd166':cape==='night-cape'?'#1b2552':'#00a7ff'}" opacity=".78"/>`; const glassEl=glasses==='visor'?`<rect x="45" y="54" width="70" height="22" rx="11" fill="#10182d" opacity=".9"/><circle cx="62" cy="65" r="4" fill="#77f7d2"/><circle cx="98" cy="65" r="4" fill="#77f7d2"/>`:glasses==='stars'?`<path d="M58 48l4 9 10 1-8 6 3 10-9-5-9 5 3-10-8-6 10-1 4-9Zm44 0l4 9 10 1-8 6 3 10-9-5-9 5 3-10-8-6 10-1 4-9Z" fill="#ffd166"/>`:`<circle cx="62" cy="64" r="19" fill="none" stroke="#14213d" stroke-width="5"/><circle cx="98" cy="64" r="19" fill="none" stroke="#14213d" stroke-width="5"/><path d="M81 64h-2" stroke="#14213d" stroke-width="5"/>`; const acc=accessory==='coffee'?`<rect x="116" y="96" width="20" height="22" rx="5" fill="#fff"/><path d="M136 101c10 1 10 12 0 13" fill="none" stroke="#fff" stroke-width="5"/><path d="M120 93c0-8 8-8 8-16" stroke="#fff" stroke-width="4" stroke-linecap="round" opacity=".75"/>`:accessory==='shield'?`<path d="M126 92l19 7v18c0 13-8 23-19 29-11-6-19-16-19-29V99l19-7Z" fill="#18c29c" stroke="#fff" stroke-width="4"/>`:accessory==='crown'?`<path d="M52 35l13-14 15 17 17-17 14 14v15H52V35Z" fill="#ffd166" stroke="#fff" stroke-width="4"/>`:`<rect x="114" y="94" width="27" height="36" rx="5" fill="#fff"/><path d="M120 104h14M120 114h14" stroke="#7c4dff" stroke-width="3" stroke-linecap="round"/>`; return `<svg class="avatar-svg ${size}" viewBox="0 0 180 180" aria-label="User avatar" role="img"><defs><filter id="avShadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="10" stdDeviation="8" flood-color="#111827" flood-opacity=".22"/></filter></defs>${capePath}<circle cx="90" cy="90" r="72" fill="${color}" filter="url(#avShadow)"/><path d="M42 88c-15-8-23-23-21-41 22 4 39 18 44 42-8 5-16 4-23-1ZM138 88c15-8 23-23 21-41-22 4-39 18-44 42 8 5 16 4 23-1Z" fill="#fff" opacity=".22"/><ellipse cx="62" cy="65" rx="26" ry="28" fill="#fff"/><ellipse cx="98" cy="65" rx="26" ry="28" fill="#fff"/><circle cx="62" cy="67" r="10" fill="#16213e"/><circle cx="98" cy="67" r="10" fill="#16213e"/><circle cx="58" cy="62" r="3" fill="#fff"/><circle cx="94" cy="62" r="3" fill="#fff"/>${glassEl}<path d="M84 87h12l-6 10-6-10Z" fill="#ffb347"/><path d="${smile}" fill="none" stroke="#fff" stroke-width="6" stroke-linecap="round" opacity=".9"/>${acc}</svg>`; }
