...
import { RESOURCE_LINKS, TIMEZONES, DOMAIN_TOPICS, CISA_RULES, DECISION_PROMPTS, HOMEWORK_SUGGESTIONS, DAILY_QUEST_TEMPLATES, SHOP_ITEMS, AVATAR_ITEMS, GAME_CATALOG } from './content.js';
...
function defaultProfile(user){
  ...
  return {
    ...
    preferences:{theme:'dark',onboardingComplete:false,pageTours:{},defaultStudyDuration:60,dailyQaeGoal:10,calendarProvider:'ics',googleConnected:false,links:{qae:RESOURCE_LINKS.qae,youtubePlaylist:RESOURCE_LINKS.youtubePlaylist,quizlet:'',goodnotes:''}},
    ...
    activity:[],daily:{date:todayIso(),quests:{}},qaeLogs:[],mistakes:[],homework:[],notes:[],quizletLogs:[],calendarEvents:[],roadmap:{startDate:todayIso(),examDate:'2026-09-26',pauseBlocks:[],bonusSessions:[],completedTasks:{}},guildIds:[],activeGroupId:null
  };
}
...
function migrateProfile(p,user){
  ...
  m.preferences.links ||= {}; m.preferences.links.qae ||= RESOURCE_LINKS.qae; m.preferences.links.youtubePlaylist ||= RESOURCE_LINKS.youtubePlaylist;
  ...
  m.activity ||= []; m.qaeLogs ||= []; m.mistakes ||= []; m.homework ||= []; m.notes ||= []; m.quizletLogs ||= []; m.calendarEvents ||= [];
...
function defaultGroup(ownerProfile, name='Study Guild'){
  ...
  return {id,name,code:Math.random().toString(36).slice(2,8).toUpperCase(),icon:'guild',color:'#7c4dff',ownerUid:ownerProfile.uid,createdAt:new Date().toISOString(),members:{[ownerProfile.uid]:publicSummary(ownerProfile)},events:[],liveSession:defaultLiveSession(ownerProfile),sessionStats:{completed:0,minutes:0,streak:0,lastDate:null},notes:[]};
}
function defaultLiveSession(profile){ return {title:'Morning Study Session',durationMinutes:profile?.preferences?.defaultStudyDuration||60,active:false,startedAt:null,accumulatedSeconds:0,completed:false,flow:dailySessionFlow(),checked:{},sharedNotes:'',updatedAt:new Date().toISOString()}; }
function dailySessionFlow(){ return [
  {id:'warmup',title:'5-Minute Recall Warm-Up',details:'Say what you remember from yesterday and name one CISA rule.',links:[{label:'Open Notebook',go:'notebook'}]},
  {id:'learn',title:'15-Minute Lesson Segment',details:'Watch or review today’s planned topic. Keep notes short and exam-focused.',links:[{label:'Open YouTube Playlist',url:RESOURCE_LINKS.youtubePlaylist},{label:'Open Exam Outline Doc',url:RESOURCE_LINKS.isacaOutline}]},
  {id:'draw',title:'10-Minute Visual Map',details:'Draw the concept flow: owner, risk, control, evidence, and auditor response.',links:[{label:'Open Notebook',go:'notebook'}]},
  {id:'qae',title:'20-Minute QAE Practice',details:'Complete questions in ISACA QAE, then log your score in Practice Log.',links:[{label:'Open ISACA QAE',url:RESOURCE_LINKS.qae},{label:'Open Practice Log',go:'practice'}]},
  {id:'review',title:'10-Minute Explain And Assign Homework',details:'Explain why misses were wrong, add Mistake Forge entries, and assign homework.',links:[{label:'Open Practice Log',go:'practice'}]}
]; }
...
function homeworkOption(h){ return `<button class="homework-card" data-hw-option="${h.id}"><div class="tagline"><span class="bubble">${h.type}</span><span class="bubble good">+${h.xp} XP</span><span class="bubble warn">+${h.coins} Coins</span></div><h4>${h.title}</h4><p class="helper">${h.details}</p></button>`; }
...
async function ensureGroupForRoom(){ if(!state.group){ const g=defaultGroup(state.profile,'My Study Guild'); state.group=g; state.profile.activeGroupId=g.id; state.profile.guildIds=[...new Set([...(state.profile.guildIds||[]),g.id])]; await createGroup(g); await saveProfileDebounced(true); } state.group.liveSession ||= defaultLiveSession(state.profile); }
async function saveSessionSettings(){ await ensureGroupForRoom(); const dur=Number($('#sessionDuration').value); if(!dur||dur<5||dur>240)return toast('Duration must be between 5 and 240 minutes.','error'); state.group.liveSession.title=$('#sessionTitle').value.trim()||'Study Session'; state.group.liveSession.durationMinutes=dur; await saveGroup(state.group); toast('Session settings saved.'); renderApp(); }
async function toggleTimer(){ await ensureGroupForRoom(); const s=state.group.liveSession; if(s.active){ s.accumulatedSeconds=timerSeconds(s); s.active=false; s.startedAt=null; } else { s.active=true; s.startedAt=new Date().toISOString(); } await saveGroup(state.group); renderApp(); }
...
function addHomework(id){ const h=HOMEWORK_SUGGESTIONS.find(x=>x.id===id); if(!h) return; state.profile.homework.unshift({id:crypto.randomUUID(),title:h.title,details:h.details,type:h.type,xp:h.xp,coins:h.coins,links:h.links,dueDate:shiftDate(todayIso(),1),createdAt:new Date().toISOString(),complete:false,source:'Study Room'}); saveProfileDebounced(); renderApp(); }
...
function buildRoadmap(){
  ...
  const weekStart=startOfWeek(start); const allTopics=[]; DOMAIN_TOPICS.forEach(d=>d.topics.forEach((t,i)=>allTopics.push({domain:d.domain,name:d.name,topic:t,domainId:d.id,topicIndex:i,links:topicLinks(t)})));
...
function lessonTasks(topic,iso){ return [
  {id:`${iso}-watch`,title:`Watch / Review: ${topic.topic}`,details:`Focus Area: ${topic.name}. Capture 3 exam-useful takeaways.`,links:topic.links},
  {id:`${iso}-map`,title:'Draw The Visual Map',details:'Map the process, owner, risk, control, evidence, and CISA trap.',links:[{label:'Open Notebook',go:'notebook'}]},
  {id:`${iso}-qae`,title:`Complete ${state.profile.preferences.dailyQaeGoal||10} QAE Questions`,details:'Use ISACA QAE for this topic/domain. Log results in Practice Log.',links:[{label:'Open ISACA QAE',url:RESOURCE_LINKS.qae},{label:'Log QAE Results',go:'practice'}]},
  {id:`${iso}-review`,title:'Review Misses And Create Rules',details:'Convert missed questions into CISA rules or Mistake Forge entries.',links:[{label:'Open Practice Log',go:'practice'}]}
]; }
...
function topicLinks(t){ return [{label:'Open YouTube Playlist',url:RESOURCE_LINKS.youtubePlaylist},{label:'Open ISACA QAE',url:RESOURCE_LINKS.qae},{label:'Open Outline Doc',url:RESOURCE_LINKS.isacaOutline}]; }
...
function dayComplete(d){ const required=d.tasks.filter(t=>!t.id.startsWith('pause')); return required.length>0 && required.every(t=>state.profile.roadmap.completedTasks[t.id]); }
...
function renderPractice(){ const total=state.profile.stats.qaeQuestions||0, correct=state.profile.stats.qaeCorrect||0, acc=qaeAccuracy();
  return `<div class="grid">
    ...
    <div class="panel"><div class="section-head"><div><p class="eyebrow">Mistake Review</p><h3>Convert Misses Into Rules</h3><p class="helper">Use this when a QAE explanation teaches you a reusable audit rule. Reviewed means you revisited it and can explain the logic.</p></div></div>${mistakeForm()}<div class="log-table">${state.profile.mistakes.length?state.profile.mistakes.map(mistakeRow).join(''):'<div class="empty">No mistakes logged yet.</div>'}</div></div>
  </div>`; }
function bindPractice(){ const d=$('#qaeDomain'); if(d){d.onchange=fillTopicSelect;fillTopicSelect();} $('#addQae')?.addEventListener('click',addQaeLog); $$('[data-edit-qae]').forEach(b=>b.onclick=()=>editQae(b.dataset.editQae)); $$('[data-delete-qae]').forEach(b=>b.onclick=()=>deleteQae(b.dataset.deleteQae)); $('#addMistake')?.addEventListener('click',addMistake); $$('[data-review-mistake]').forEach(b=>b.onclick=()=>reviewMistake(b.dataset.reviewMistake)); $$('[data-edit-mistake]').forEach(b=>b.onclick=()=>editMistake(b.dataset.editMistake)); $$('[data-delete-mistake]').forEach(b=>b.onclick=()=>deleteMistake(b.dataset.deleteMistake)); }
function fillTopicSelect(){ const d=DOMAIN_TOPICS.find(x=>x.id===$('#qaeDomain').value); $('#qaeTopic').innerHTML=(d?.topics||[]).map(t=>`<option>${t}</option>`).join(''); }
...
function renderTools(){
  return `<div class="grid">
    <div class="panel" data-tour="tool-integrations"><div class="section-head"><div><p class="eyebrow">Study Tool Hub</p><h3>Use The Best Tool For Each Job</h3><p class="helper">This page does not replace ISACA QAE, Quizlet, or Goodnotes. It connects them and tracks the extra layer: practice, games, confidence, and Guild accountability.</p></div></div>
      <div class="grid three">
        ${integrationCard('ISACA Learning / QAE','Use this for official practice questions and course modules. Log results in Practice Log.','qae',RESOURCE_LINKS.qae,'Open ISACA')}
        ${integrationCard('Quizlet','Keep flashcards in Quizlet. Add deck links and log your study rounds here.','cards',state.profile.preferences.links.quizlet||RESOURCE_LINKS.quizlet,'Open Quizlet')}
        ${integrationCard('Goodnotes','Use a shared Goodnotes link for handwritten notes. Guild notes still save in ControlQuest Notebook.','notebook',state.profile.preferences.links.goodnotes||RESOURCE_LINKS.goodnotes,'Open Goodnotes')}
      </div>
    </div>
    <div class="panel"><div class="section-head"><div><p class="eyebrow">Quizlet Tracker</p><h3>Log Flashcard Practice</h3></div></div><div class="form-grid"><input id="quizletDeck" placeholder="Deck Name Or Link"><input id="quizletMinutes" type="number" min="1" max="240" value="15"><select id="quizletConfidence"><option>Low Confidence</option><option selected>Medium Confidence</option><option>High Confidence</option></select></div><textarea id="quizletNotes" placeholder="What terms were shaky?"></textarea><button class="primary-button" id="logQuizlet">Log Quizlet Practice</button><div class="log-table">${state.profile.quizletLogs.length?state.profile.quizletLogs.slice(0,8).map(q=>`<div class="log-row"><div><strong>${esc(q.deck)}</strong><p class="helper">${q.minutes} Minutes · ${q.confidence} · ${fmtDate(q.date)}</p><p>${esc(q.notes||'')}</p></div></div>`).join(''):'<div class="empty">No Quizlet logs yet.</div>'}</div></div>
    <div class="panel" data-tour="arcade"><div class="section-head"><div><p class="eyebrow">ControlQuest Arcade</p><h3>Original Games For Audit Judgment</h3><p class="helper">These games are intentionally different from flashcard games. They train sequence, risk ranking, controls, and CISA “best answer” logic.</p></div></div><div class="grid three">${GAME_CATALOG.map(gameCard).join('')}</div><div id="gameArena" style="margin-top:16px">${renderGameArena()}</div></div>
  </div>`;
}
...
function renderGameArena(){ if(!state.gameState)return `<div class="empty">Pick a game above to start a quick challenge.</div>`; const g=state.gameState; if(g.type==='avatarSprint') return renderAvatarSprint(); if(g.type==='riskRank') return renderRiskRank(); if(g.type==='sequence') return renderSequenceGame(); if(g.type==='controlMatch') return renderControlMatch(); if(g.type==='duel') return renderDecisionDuel(); return ''; }
function bindTools(){ $('#logQuizlet')?.addEventListener('click',logQuizlet); $$('[data-start-game]').forEach(b=>b.onclick=()=>startGame(b.dataset.startGame)); $$('[data-game-answer]').forEach(b=>b.onclick=()=>gameAnswer(Number(b.dataset.gameAnswer))); $('#riskSubmit')?.addEventListener('click',submitRiskRank); $('#seqSubmit')?.addEventListener('click',submitSequence); $('#matchSubmit')?.addEventListener('click',submitControlMatch); }
function logQuizlet(){ const deck=$('#quizletDeck').value.trim(); const mins=Number($('#quizletMinutes').value); if(!deck)return toast('Add a Quizlet deck name or link.','error'); if(!mins||mins<1||mins>240)return toast('Minutes must be 1 to 240.','error'); state.profile.quizletLogs.unshift({id:crypto.randomUUID(),date:todayIso(),deck,minutes:mins,confidence:$('#quizletConfidence').value,notes:$('#quizletNotes').value.trim()}); award({xp:Math.min(60,10+mins),coins:Math.min(25,4+Math.round(mins/3)),reason:'Quizlet Practice Logged',type:'Quizlet'}); saveProfileDebounced(); renderApp(); }
function startGame(id){ state.gameState={type:id,round:0,score:0,startedAt:Date.now(),prompt:pick(DECISION_PROMPTS)}; renderApp(); }
...
function renderProfile(){ return `<div class="grid"><div class="panel"><div class="section-head"><div><p class="eyebrow">Profile And Preferences</p><h3>Customize Your Setup</h3></div></div><div class="form-grid"><label><span class="label-title">Display Name</span><input id="displayName" value="${escAttr(state.profile.displayName)}"></label><label><span class="label-title">Timezone</span><select id="timezoneSelect">${TIMEZONES.map(t=>`<option value="${t}" ${t===state.profile.timezone?'selected':''}>${t}</option>`).join('')}</select></label><label><span class="label-title">Theme</span><select id="prefTheme"><option value="dark" ${state.profile.preferences.theme==='dark'?'selected':''}>Dark</option><option value="light" ${state.profile.preferences.theme==='light'?'selected':''}>Light</option></select></label><label><span class="label-title">Default Study Duration</span><input id="prefDuration" type="number" min="5" max="240" value="${state.profile.preferences.defaultStudyDuration||60}"></label></div><button class="primary-button" id="saveProfile">Save Profile</button></div>
    <div class="panel"><p class="eyebrow">External Tool Links</p><div class="form-grid"><label><span class="label-title">ISACA QAE / Learning Link</span><input id="linkQae" value="${escAttr(state.profile.preferences.links.qae||RESOURCE_LINKS.qae)}"></label><label><span class="label-title">YouTube Playlist Link</span><input id="linkYoutube" value="${escAttr(state.profile.preferences.links.youtubePlaylist||RESOURCE_LINKS.youtubePlaylist)}"></label><label><span class="label-title">Quizlet Link</span><input id="linkQuizlet" value="${escAttr(state.profile.preferences.links.quizlet||'')}"></label><label><span class="label-title">Goodnotes Share Link</span><input id="linkGoodnotes" value="${escAttr(state.profile.preferences.links.goodnotes||'')}"></label></div><button class="primary-button" id="saveLinks">Save Links</button></div>
...
function showOnboarding(){
  ...
  modal('Welcome To ControlQuest Studio', `<p class="helper">Let’s set up your personal CISA study companion. These choices are private to your profile and will not change your Guild members’ settings.</p>
    ...
    <label><span class="label-title">Quizlet Link</span><input id="obQuizlet" placeholder="Optional Quizlet folder or deck link" value="${escAttr(p.preferences.links.quizlet||'')}"></label>
    <label><span class="label-title">Goodnotes Share Link</span><input id="obGoodnotes" placeholder="Optional Goodnotes web share link" value="${escAttr(p.preferences.links.goodnotes||'')}"></label>`, async()=>{
      ...
      p.displayName=$('#obName').value.trim()||p.displayName; p.preferences.theme=$('#obTheme').value; p.timezone=$('#obTimezone').value; p.roadmap.startDate=start; p.roadmap.examDate=exam; p.preferences.defaultStudyDuration=dur; p.preferences.dailyQaeGoal=goal; p.preferences.calendarProvider=$('#obCalendar').value; p.preferences.links.quizlet=$('#obQuizlet').value.trim(); p.preferences.links.goodnotes=$('#obGoodnotes').value.trim(); p.preferences.onboardingComplete=true; p.preferences.pageTours ||= {}; await saveProfileDebounced(true); applyTheme(); renderApp(); setTimeout(()=>startTour('command',true),400);
    }, 'Start My Quest');
}
...
function avatarSvg(eq={}){ const color=eq.baseColor||'#2fb7ff'; const eyes=eq.eyes||'normal', glasses=eq.glasses||'none', cape=eq.cape||'none', hat=eq.hat||'none', badge=eq.badge||'none'; return `<svg viewBox="0 0 200 200" role="img" aria-label="Audit Owl Avatar"><defs><linearGradient id="owlG" x1="0" x2="1" y1="0" y2="1"><stop stop-color="${color}"/><stop offset="1" stop-color="#00c2ff"/></linearGradient></defs>${cape!=='none'?`<path d="M60 86c-26 34-34 70-20 92 25-14 44-14 60-4 16-10 35-10 60 4 14-22 6-58-20-92Z" fill="${cape==='gold'?'#ffd166':'#4169ff'}" opacity=".88"/>`:''}<circle cx="100" cy="96" r="72" fill="#dfe7f1"/><circle cx="100" cy="92" r="61" fill="url(#owlG)"/><circle cx="76" cy="74" r="26" fill="#fff"/><circle cx="124" cy="74" r="26" fill="#fff"/>${eyeSvg(eyes)}<path d="M100 88l16 20H84z" fill="#ffd166"/><path d="M72 117c18 14 38 14 56 0" stroke="#eef6ff" stroke-width="9" stroke-linecap="round" fill="none"/><rect x="60" y="130" width="80" height="16" rx="8" fill="rgba(0,0,0,.15)"/><path d="M78 142l-16 38M122 142l16 38" stroke="#ffb347" stroke-width="13" stroke-linecap="round"/>${glassesSvg(glasses)}${hatSvg(hat)}${badgeSvg(badge)}</svg>`; }
